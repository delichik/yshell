...
- 右键菜单设计矩阵：`docs/product/context-menu-design.md`
...
- 支持查找、正则查找、跨 SSH 查找、结果高亮、复制、复制当前行、复制屏幕内容、粘贴、矩形选择、全选屏幕内容、全选全部终端内容、清屏、软换行。
...
- 支持保存的 session 复制能力：Duplicate、Duplicate Without Secrets、Duplicate to Folder、Copy Connection String、Copy Host、Copy User@Host。
...
- 支持单窗口多标签。
- 支持标签拖拽排序、关闭、复制会话、重连。
- 支持标签右键关闭族操作：关闭当前、关闭其他、关闭左侧、关闭右侧、关闭全部、关闭断开的标签。
- 支持左右/上下分屏标签组。
